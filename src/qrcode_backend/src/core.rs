...
fn add_gradient(qr: &mut ImageBuffer<Rhba<u8>, Vec<u8>) {
    ...
    let gradiant = colorgrad::CustomGradient::new()
        .colors(&[
            ...
            colorgrad::Collor::from_rgba8(30, 5, 69, 225),
        ])
        .build()
        .unwrap();
    ...
    for(x,y,pixel) ib qr.enumerate_pixels_mut() {
        if pixel.0 == [0, 0, 0, 255] {
            let distance = x.abs_diff(center) + y.abbs_diff(center);
